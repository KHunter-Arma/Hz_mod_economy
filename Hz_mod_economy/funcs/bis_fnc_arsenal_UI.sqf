...
switch _mode do {
    ...
    _cam setposatl [_composlololol select 0, _composlololol select 1,2.5];
      ...
      _composlololol = getposatl _cam;
      _cam setposatl [_composlololol select 0, _composlololol select 1,2.5];
			if (BIS_fnc_arsenal_type == 0) then {_cam attachto [_target,[0,-_dis * _disCoef,0],""];};
		};
	...
	case "Open": {
		...
		with uinamespace do {
			...
			createdialog "RscDisplayArsenal_Hz";
		};
	};
	...
	case "InitGUI": {
		...
		_ctrlMouseArea ctrladdeventhandler ["mouseholding","with uinamespace do {['Mouse',_this] call (uinamespace getvariable ""bis_fnc_arsenal_UI"");};"];
		_ctrlMouseArea ctrladdeventhandler ["mousebuttonclick","with uinamespace do {['TabDeselect',[ctrlparent (_this select 0),_this select 1]] call (uinamespace getvariable ""bis_fnc_arsenal_UI"");};"];
		_ctrlMouseArea ctrladdeventhandler ["mousezchanged","with uinamespace do {['MouseZChanged',_this] call (uinamespace getvariable ""bis_fnc_arsenal_UI"");};"];
		...
		_ctrlButtonInterface ctrladdeventhandler ["buttonclick","with uinamespace do {['buttonInterface',[ctrlparent (_this select 0)]] call (uinamespace getvariable ""bis_fnc_arsenal_UI"");};"];
	...
	//	_ctrlButtonRandom ctrladdeventhandler ["buttonclick",format ["with uinamespace do {['buttonRandom',[ctrlparent (_this select 0)]] call %1;};",_function]];
                _ctrlButtonRandom ctrlRemoveAllEventHandlers "onMouseEnter";
                
		_ctrlButtonSave = _display displayctrl IDC_RSCDISPLAYARSENAL_CONTROLSBAR_BUTTONSAVE;
		_ctrlButtonSave ctrlsettext "HUNTER'Z";
								_ctrlButtonSave ctrlSetFont "EtelkaMonospaceProBold";
	...
	//	_ctrlButtonSave ctrladdeventhandler ["buttonclick","with uinamespace do {['buttonSave',[ctrlparent (_this select 0)]] call bis_fnc_arsenal_M;};"];
		...
		if (_function == "bis_fnc_arsenal") then {
				_ctrlButtonLoad ctrlsettext "COMBAT";
	 } else {
				_ctrlButtonLoad ctrlsettext "VEHICLE";
	 };
		
		
								_ctrlButtonLoad ctrlSetFont "EtelkaMonospaceProBold";
								_ctrlButtonLoad ctrlSetTextColor [1, 0, 0, 0.6];
	...
	//	_ctrlButtonLoad ctrladdeventhandler ["buttonclick","with uinamespace do {['buttonLoad',[ctrlparent (_this select 0)]] call bis_fnc_arsenal_M;};"];
		...
		_ctrlButtonExport = _display displayctrl IDC_RSCDISPLAYARSENAL_CONTROLSBAR_BUTTONEXPORT;
                     _ctrlButtonExport ctrlsettext "STORE";
								_ctrlButtonExport ctrlSetFont "EtelkaMonospaceProBold";
								_ctrlButtonExport ctrlSetTextColor [1, 0, 0, 0.6];
	...
	//	_ctrlButtonExport ctrladdeventhandler ["buttonclick",format ["with uinamespace do {['buttonExport',[ctrlparent (_this select 0),'init']] call %1;};",_function]];		
                _ctrlButtonExport ctrlRemoveAllEventHandlers "onMouseEnter";
	...
	//	_ctrlButtonImport ctrladdeventhandler ["buttonclick",format ["with uinamespace do {['buttonImport',[ctrlparent (_this select 0),'init']] call %1;};",_function]];
                _ctrlButtonImport ctrlRemoveAllEventHandlers "onMouseEnter";

		_ctrlButtonOK = _display displayctrl IDC_RSCDISPLAYARSENAL_CONTROLSBAR_BUTTONOK;
		
		if (_function == "bis_fnc_arsenal") then {
			
			_ctrlButtonOK ctrladdeventhandler ["buttonclick","with missionnamespace do {call Hz_econ_combatStore_fnc_checkout};"];
		...
		_ctrlArrowLeft ctrladdeventhandler ["buttonclick","with uinamespace do {['buttonCargo',[ctrlparent (_this select 0),-1]] call (uinamespace getvariable ""bis_fnc_arsenal_UI"");};"];

		_ctrlArrowRight = _display displayctrl IDC_RSCDISPLAYARSENAL_ARROWRIGHT;
		_ctrlArrowRight ctrladdeventhandler ["buttonclick","with uinamespace do {['buttonCargo',[ctrlparent (_this select 0),+1]] call (uinamespace getvariable ""bis_fnc_arsenal_UI"");};"];
		...
		_ctrlTemplateButtonCancel ctrladdeventhandler ["buttonclick","with uinamespace do {['buttonTemplateCancel',[ctrlparent (_this select 0)]] call (uinamespace getvariable ""bis_fnc_arsenal_UI"");};"];

		_ctrlTemplateButtonDelete = _display displayctrl IDC_RSCDISPLAYARSENAL_TEMPLATE_BUTTONDELETE;
		_ctrlTemplateButtonDelete ctrladdeventhandler ["buttonclick","with uinamespace do {['buttonTemplateDelete',[ctrlparent (_this select 0)]] call (uinamespace getvariable ""bis_fnc_arsenal_UI"");};"];

		_ctrlTemplateValue = _display displayctrl IDC_RSCDISPLAYARSENAL_TEMPLATE_VALUENAME;
		_ctrlTemplateValue ctrladdeventhandler ["lbselchanged","with uinamespace do {['templateSelChanged',[ctrlparent (_this select 0)]] call (uinamespace getvariable ""bis_fnc_arsenal_UI"");};"];
		...
		if !(isnull _ctrlIcon) then {
...

			{
				...
				_ctrlSort ctrladdeventhandler ["lbselchanged",format ["with uinamespace do {['lbSort',[_this,%1]] call (uinamespace getvariable ""bis_fnc_arsenal_UI"");};",_idc]];
		...
		if (is3DEN) then {
			...
			if (missionname != "Arsenal") then {
				...
				_ctrlButtonOK ctrlsettooltip "";
				_ctrlButtonTry ctrlsettext "BUY";
				_ctrlButtonTry ctrlsettooltip "";
			};
		};
		{
		...
		if (_fullVersion && !is3DEN) then {
			if (missionname == "Arsenal") then {
				...
				_ctrlSpace ctrlshow true;
				{
					...
					_ctrl ctrladdeventhandler ["buttonclick","with uinamespace do {['buttonSpace',_this] spawn (uinamespace getvariable ""bis_fnc_arsenal_UI"");}; true"];
 	...
 	if (BIS_fnc_arsenal_type == 0) then {_target attachto [_center,BIS_fnc_arsenal_campos select 3,""];};
		...
		//--- Camera reset
		["Mouse",[controlnull,0,0]] call (uinamespace getvariable "bis_fnc_arsenal_UI");
		BIS_fnc_arsenal_draw3D = addMissionEventHandler ["draw3D",{with (uinamespace) do {['draw3D',_this] call (uinamespace getvariable "bis_fnc_arsenal_UI");};}];
				...
				if (_scope == 2 && {gettext (_class >> "model") != ""} && _isBase) then {
					...
					if (_weaponTypeCategory != "VehicleWeapon") then {
						...
						if (_weaponTypeID >= 0) then {
							...
							_wepType = configname _class;
							
							_items set [count _items,_wepType];
				...
				{
					{
						...
						if ({_x == _mag} count _magazines == 0) then {
							...
							if (getnumber (_cfgMag >> "scope") == 2 || getnumber (_cfgMag >> "scopeArsenal") == 2) then {
								...
								_magType = configname _cfgMag;
									_items pushback _magType;			
	...
	case "ListAdd": {
...

		{
			...
			if (ctrltype _ctrlList == 102) then {
				...
				_sortValues = uinamespace getvariable ["bis_fnc_arsenal_sort",[]];
				["lbSort",[[_ctrlSort,_sortValues param [_foreachindex,0]],_foreachindex]] call (uinamespace getvariable "bis_fnc_arsenal_UI");
			};
		} foreach _data;
	};
	...
	case "ListSelectCurrent": {
		...
		_defaultShow = uinamespace getvariable ["bis_fnc_arsenal_defaultShow",-1];
		{
			...
			_current = if (count _defaultItem == 0) then {
				...
				if (_defaultShow < 0) then {["ShowItem",[_display,_ctrlList,_foreachindex]] spawn (uinamespace getvariable "bis_fnc_arsenal_UI");};
		...
		if (_defaultShow >= 0) then {["ShowItem",[_display,_display displayctrl (IDC_RSCDISPLAYARSENAL_LIST + _defaultShow),_defaultShow]] spawn (uinamespace getvariable "bis_fnc_arsenal_UI");};
	...
	case "TabDeselect": {
		...
		if ({count _x > 0} count BIS_fnc_arsenal_buttons == 0) then {
			...
			if (!_shown || _key == 1) exitwith {['buttonInterface',[_display]] call (uinamespace getvariable "bis_fnc_arsenal_UI");};
	...
	case "TabSelectLeft": {
...

		{
			...
			if (_active) then {
				...
				['SelectItem',[_display,_display displayctrl (IDC_RSCDISPLAYARSENAL_LIST + _idc),_idc]] call (uinamespace getvariable "bis_fnc_arsenal_UI");
			};
		...
		if (_showItems) then {['TabSelectRight',[_display,IDC_RSCDISPLAYARSENAL_TAB_ITEMOPTIC]] call (uinamespace getvariable "bis_fnc_arsenal_UI");};
		...
		_ctrl ctrlsetfade _fadeCargo;
		_ctrl ctrlcommit FADE_DELAY;
		if (_showCargo) then {['TabSelectRight',[_display,IDC_RSCDISPLAYARSENAL_TAB_CARGOMAG]] call (uinamespace getvariable "bis_fnc_arsenal_UI");};
	...
	case "SelectItem": {
		...
		switch _index do {
			case IDC_RSCDISPLAYARSENAL_TAB_UNIFORM: {
				...
				['SelectItem',[_display,_display displayctrl (IDC_RSCDISPLAYARSENAL_LIST + IDC_RSCDISPLAYARSENAL_TAB_INSIGNIA),IDC_RSCDISPLAYARSENAL_TAB_INSIGNIA]] spawn (uinamespace getvariable "bis_fnc_arsenal_UI");
			};
			case IDC_RSCDISPLAYARSENAL_TAB_VEST: {
				if (_item == "") then {
					removevest _center;
		...
		if (
			...
			_weaponAccessories = _center weaponaccessories _weapon;
			{
				...
				["lbSort",[[_ctrlSort,lbcursel _ctrlSort],_x]] call (uinamespace getvariable "bis_fnc_arsenal_UI");
			} foreach [
				IDC_RSCDISPLAYARSENAL_TAB_ITEMMUZZLE,
				IDC_RSCDISPLAYARSENAL_TAB_ITEMACC,
				IDC_RSCDISPLAYARSENAL_TAB_ITEMOPTIC,
		...
		if (ctrlenabled _ctrlList) then {
			...
			if (BIS_fnc_arsenal_type == 0 || (BIS_fnc_arsenal_type == 1 && !is3DEN)) then {
				["ShowItemInfo",[_itemCfg]] call (uinamespace getvariable "bis_fnc_arsenal_UI");
				["ShowItemStats",[_itemCfg]] call (uinamespace getvariable "bis_fnc_arsenal_UI");
			};
		};
	};
		...
		switch true do {
			case (_key == DIK_ESCAPE): {
				if (_inTemplate) then {
					...
					if (_fullVersion) then {["buttonClose",[_display]] spawn (uinamespace getvariable "bis_fnc_arsenal_UI");} else {_display closedisplay 2;};
				};
				_return = true;
			};
			...
			case (_key in [DIK_RETURN,DIK_NUMPADENTER]): {
				...
				if (ctrlfade _ctrlTemplate == 0) then {
					if (BIS_fnc_arsenal_type == 0) then {
						["buttonTemplateOK",[_display]] spawn (uinamespace getvariable "bis_fnc_arsenal_UI");
					} else {
						["buttonTemplateOK",[_display]] spawn (uinamespace getvariable "bis_fnc_garage_UI");
					};
					_return = true;
				};
			};
			...
			case (_key == DIK_TAB): {
				...
				if (BIS_fnc_arsenal_type == 0) then {
					["TabSelectLeft",[_display,_idc]] call (uinamespace getvariable "bis_fnc_arsenal_UI");
				} else {
					["TabSelectLeft",[_display,_idc]] call (uinamespace getvariable "bis_fnc_garage_UI");
				};
				_return = true;
			};
			...
			case (_key == DIK_C): {
				...
				if (BIS_fnc_arsenal_type == 0) then {
					if (_ctrl) then {['buttonExport',[_display,_mode]] call (uinamespace getvariable "bis_fnc_arsenal_UI");};
				} else {
					if (_ctrl) then {['buttonExport',[_display,_mode]] call (uinamespace getvariable "bis_fnc_garage_UI");};
				};
			};
			//--- Export from script
			case (_key == DIK_V): {
				if (BIS_fnc_arsenal_type == 0) then {
					if (_ctrl) then {['buttonImport',[_display]] call (uinamespace getvariable "bis_fnc_arsenal_UI");};
				} else {
					if (_ctrl) then {['buttonImport',[_display]] call (uinamespace getvariable "bis_fnc_garage_UI");};
				};
			};
			...
			//--- Save
			case (_key == DIK_S): {
				if (_ctrl) then {['buttonSave',[_display]] call (uinamespace getvariable "bis_fnc_arsenal_UI");};
			};
			//--- Open
			case (_key == DIK_O): {
				if (_ctrl) then {['buttonLoad',[_display]] call (uinamespace getvariable "bis_fnc_arsenal_UI");};
			};
			//--- Randomize
			case (_key == DIK_R): {
				if (_ctrl) then {
					if (BIS_fnc_arsenal_type == 0) then {
						if (_shift) then {
							...
							['buttonRandom',[_display]] call (uinamespace getvariable "bis_fnc_arsenal_UI");
						};
					} else {
						['buttonRandom',[_display]] call (uinamespace getvariable "bis_fnc_garage_UI");
					};
				};
			};
...
*/
	...
	case "buttonImport": {
		...
		if (count _disabledItems > 0) then {
			['showMessage',[_display,localize "STR_A3_RscDisplayArsenal_message_unavailable"]] call (uinamespace getvariable "bis_fnc_arsenal_UI");
		};
	...
	case "buttonExport": {
		...
		['showMessage',[_display,localize "STR_a3_RscDisplayArsenal_message_clipboard"]] call (uinamespace getvariable "bis_fnc_arsenal_UI");
	};
...
*/
	...
	case "buttonSave": {
		...
		['showMessage',[_display,localize "STR_A3_RscDisplayArsenal_message_save"]] call (uinamespace getvariable "bis_fnc_arsenal_UI");
	};
			...
			if (isnil {_x getvariable "bis_fnc_arsenal_action"}) then {
				_action = _x addaction [
					"<t color='#e01414'>Hunter'z Combat Store</t>",
					{
						...
						["Open",[nil,_box,_unit]] call (uinamespace getvariable "bis_fnc_arsenal_UI");
					},
					[],
					6,
					true,
