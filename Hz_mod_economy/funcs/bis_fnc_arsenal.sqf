...
("isclass _x && getnumber (_x >> 'scope') == 2 && getnumber (_x >> 'type') < 5") configclasses (configfile >> "cfgweapons"),
	["reloadtime","dispersion","maxzeroing","hit","mass","initSpeed"],	[true,true,false,true,false,false]
...
("isclass _x && getnumber (_x >> 'scope') == 2 && getnumber (_x >> 'itemInfo' >> 'type') in [605,701,801]") configclasses (configfile >> "cfgweapons"),
	["passthrough","armor","maximumLoad","mass"],	[false,false,false,false]
] call bis_fnc_configExtremes;
_statsBackpacks = [
...
if ((_function == "bis_fnc_arsenal") || (_function == "bis_fnc_arsenal_M")) then {

_function = "bis_fnc_arsenal_UI";

};

_display displayaddeventhandler ["mousebuttondown","with uinamespace do {['MouseButtonDown',_this] call bis_fnc_arsenal_UI;};"];
_display displayaddeventhandler ["mousebuttonup","with uinamespace do {['MouseButtonUp',_this] call bis_fnc_arsenal_UI;};"];

_display displayaddeventhandler ["keydown","with (uinamespace) do {['KeyDown',_this] call bis_fnc_arsenal_UI;};"];



_ctrlMouseArea = _display displayctrl 			899;
_ctrlMouseArea ctrladdeventhandler ["mousemoving","with uinamespace do {['Mouse',_this] call bis_fnc_arsenal_UI;};"];
_ctrlMouseArea ctrladdeventhandler ["mouseholding","with uinamespace do {['Mouse',_this] call bis_fnc_arsenal_UI;};"];
_ctrlMouseArea ctrladdeventhandler ["mousebuttonclick","with uinamespace do {['TabDeselect',[ctrlparent (_this select 0),_this select 1]] call bis_fnc_arsenal_UI;};"];
_ctrlMouseArea ctrladdeventhandler ["mousezchanged","with uinamespace do {['MouseZChanged',_this] call bis_fnc_arsenal_UI;};"];
...
_ctrlButtonInterface ctrladdeventhandler ["buttonclick","with uinamespace do {['buttonInterface',[ctrlparent (_this select 0)]] call bis_fnc_arsenal_UI;};"];
...
_ctrlButtonSave ctrladdeventhandler ["buttonclick","with uinamespace do {['buttonSave',[ctrlparent (_this select 0)]] call bis_fnc_arsenal_UI;};"];

_ctrlButtonLoad = _display displayctrl 		44147;
_ctrlButtonLoad ctrladdeventhandler ["buttonclick","with uinamespace do {['buttonLoad',[ctrlparent (_this select 0)]] call bis_fnc_arsenal_UI;};"];
...
_ctrlButtonTry ctrladdeventhandler ["buttonclick","['buttonTry',[ctrlparent (_this select 0)]] call bis_fnc_garage_UI;"];

_ctrlArrowLeft = _display displayctrl 			992;
_ctrlArrowLeft ctrladdeventhandler ["buttonclick","with uinamespace do {['buttonCargo',[ctrlparent (_this select 0),-1]] call bis_fnc_arsenal_UI;};"];

_ctrlArrowRight = _display displayctrl 		993;
_ctrlArrowRight ctrladdeventhandler ["buttonclick","with uinamespace do {['buttonCargo',[ctrlparent (_this select 0),+1]] call bis_fnc_arsenal_UI;};"];
...
_ctrlTemplateButtonCancel ctrladdeventhandler ["buttonclick","with uinamespace do {['buttonTemplateCancel',[ctrlparent (_this select 0)]] call bis_fnc_arsenal_UI;};"];

_ctrlTemplateButtonDelete = _display displayctrl 	36021;
_ctrlTemplateButtonDelete ctrladdeventhandler ["buttonclick","with uinamespace do {['buttonTemplateDelete',[ctrlparent (_this select 0)]] call bis_fnc_arsenal_UI;};"];

_ctrlTemplateValue = _display displayctrl 		35119;
_ctrlTemplateValue ctrladdeventhandler ["lbselchanged","with uinamespace do {['templateSelChanged',[ctrlparent (_this select 0)]] call bis_fnc_arsenal_UI;};"];
...
_x ctrladdeventhandler ["mousezchanged","with uinamespace do {['MouseZChanged',_this] call bis_fnc_arsenal_UI;};"];
} foreach [_ctrlIcon,_ctrlTab];

_sort = _sortValues param [_idc,0];
_ctrlSort = _display displayctrl (			800 + _idc);
_ctrlSort ctrladdeventhandler ["lbselchanged",format ["with uinamespace do {['lbSort',[_this,%1]] call bis_fnc_arsenal_UI;};",_idc]];
...
_ctrlList ctrladdeventhandler ["lbdblclick",format ["with uinamespace do {['ShowItem',[ctrlparent (_this select 0),(_this select 0),%1]] spawn bis_fnc_arsenal_UI;};",_idc]];
...
_ctrlButtonClose ctrladdeventhandler ["buttonclick","with uinamespace do {['buttonClose',[ctrlparent (_this select 0)]] spawn bis_fnc_arsenal_UI;}; true"];
...
_ctrl ctrladdeventhandler ["buttonclick","with uinamespace do {['buttonSpace',_this] spawn bis_fnc_arsenal_UI;}; true"];
...
private _items = _data select 		26;
...
_lbAdd = -1;
_xCfg = configfile;
_modList = ["","curator","kart","heli","mark","expansion","expansionpremium"];
_fnc_addModIcon = 	{		private _dlcName = _this call 	{		private _dlc = "";		private _addons = configsourceaddonlist _this;		if (count _addons > 0) then {			private _mods = configsourcemodlist (configfile >> "CfgPatches" >> _addons select 0);			if (count _mods > 0) then {				_dlc = _mods select 0;			};		};		_dlc	};		if (_dlcName != "") then {			_ctrlList lbsetpictureright [_lbAdd,(modParams [_dlcName,["logo"]]) param [0,""]];			_modID = _modList find _dlcName;			if (_modID < 0) then {_modID = _modList pushback _dlcName;};			_ctrlList lbsetvalue [_lbAdd,_modID];		};	};;
_listDefaults = [
[(primaryweapon _center) call bis_fnc_baseWeapon], 
[(secondaryweapon _center) call bis_fnc_baseWeapon], 
[(handgunweapon _center) call bis_fnc_baseWeapon], 
[uniform _center], 
[vest _center], 
[backpack _center], 
[headgear _center], 
...
if (_defaultShow >= 0) then {["ShowItem",[_display,_display displayctrl (			960 + _defaultShow),_defaultShow]] spawn bis_fnc_arsenal;};
...
if (_idc in [			18,			19,			20,			25,			21,			26,			22,			23,			24]) then {
_ctrlList = _display displayctrl (			960 + _idc);
_ctrlSort = _display displayctrl (			800 + _idc);
{
_x ctrlenable false;
...
case (ctrlenabled (_display displayctrl (			960 + 		3))): {uniformitems _center};
case (ctrlenabled (_display displayctrl (			960 + 			4))): {vestitems _center};
case (ctrlenabled (_display displayctrl (			960 + 		5))): {backpackitems _center};
default {[]};
};
for "_l" from 0 to (lbsize _ctrlList - 1) do {
...
_ctrlIconBackground = _display displayctrl (		830 + _idc);
_ctrlIconBackground ctrlshow _active;
} foreach [			18,			19,			20,			25,			21,			26,			22,			23,			24];
};
...
['SelectItem',[_display,_display displayctrl (			960 + 		17),		17]] spawn bis_fnc_arsenal;
};
case 			4: {
if (_item == "") then {
removevest _center;
...
_center addweapon _item;
	_magazines = getarray (configfile >> "cfgweapons" >> _item >> "magazines");	if (count _magazines > 0) then {_center addmagazine (_magazines select 0);};
};
};
case 		0: {
...
_itemCfg call 	{		private _dlcName = _this call 	{		private _dlc = "";		private _addons = configsourceaddonlist _this;		if (count _addons > 0) then {			private _mods = configsourcemodlist (configfile >> "CfgPatches" >> _addons select 0);			if (count _mods > 0) then {				_dlc = _mods select 0;			};		};		_dlc	};		if (_dlcName != "") then {			_ctrlList lbsetpictureright [_lbAdd,(modParams [_dlcName,["logo"]]) param [0,""]];			_modID = _modList find _dlcName;			if (_modID < 0) then {_modID = _modList pushback _dlcName;};			_ctrlList lbsetvalue [_lbAdd,_modID];		};	};;
};
...
_dlc = _itemCfg call 	{		private _dlc = "";		private _addons = configsourceaddonlist _this;		if (count _addons > 0) then {			private _mods = configsourcemodlist (configfile >> "CfgPatches" >> _addons select 0);			if (count _mods > 0) then {				_dlc = _mods select 0;			};		};		_dlc	};
...
case "addweapon": {
if ((_fullVersion || {"%ALL" in _virtualWeaponCargo} || {{_item == _x} count _virtualWeaponCargo > 0})) then {
_center addweapon _item;
if (((_item call bis_fnc_itemType) select 0) == "LaserDesignator") then {	_magazines = getarray (configfile >> "cfgweapons" >> _item >> "magazines");	if (count _magazines > 0) then {_center addmagazine (_magazines select 0);};};
} else {
if !(_item in _disabledItems) then {_disabledItems set [count _disabledItems,_item];};
};
};
