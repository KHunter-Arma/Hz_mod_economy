                    ...
                    switch _mode do {
                            case "onLoad": {
                                    if (isnil {missionnamespace getvariable "bis_fnc_arsenal_data"}) then {
                                           startloadingscreen [""];
                                            ['Init',_params] spawn compile preprocessfilelinenumbers (Hz_econ_funcsPath + "bis_fnc_arsenal_UI.sqf");
                                    } else {
                                            ['Init',_params] call compile preprocessfilelinenumbers (Hz_econ_funcsPath + "bis_fnc_arsenal_UI.sqf");
                                    };
                            };
                            case "onUnload": {
                                    ['Exit',_params] call compile preprocessfilelinenumbers (Hz_econ_funcsPath + "bis_fnc_arsenal_UI.sqf");
                            };
                    };       
