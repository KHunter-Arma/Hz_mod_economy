...
private _HzAmmoUnitCost = _HzAmmoClass call Hz_econ_combatStore_fnc_getAmmoPrice;
...
if (_maxMagazines == 1) then {
	...
	if (GVAR(level) == 1) then {
		...
		// Hunter'z Economy Interface
		private _HzRoundsAdded = ((_vehicle magazineTurretAmmo [_magazineClass, _turretPath]) + _numRounds) min _rounds;
		private _HzCost = _HzRoundsAdded*_HzAmmoUnitCost;
		...
		if (Hz_econ_funds < _HzCost) then {
			...
			_vehicle setMagazineTurretAmmo [_magazineClass,_HzRoundsAdded, _turretPath];
	...
	// Hunter'z Economy Interface
	private _HzRoundsAdded = _maxMagazines*_rounds;
	private _HzCost = _HzRoundsAdded*_HzAmmoUnitCost;
	
	if (Hz_econ_funds < _HzCost) exitwith {"Insufficient funds!" remoteExecCall ["hint",_unit,false];};		
		...
		if (_currentRounds > 0 || {_idx == (_maxMagazines+1)}) exitWith {
			...
			if (GVAR(level) == 2) then {
				...
				if ((_currentRounds + _numRounds) > _rounds) then {
					_vehicle setMagazineTurretAmmo [_magazineClass, _rounds, _turretPath];
					_HzCost = _HzCost + (_rounds - _currentRounds)*_HzAmmoUnitCost;
					if (_numMagazines  < _maxMagazines) then {
						_vehicle addMagazineTurret [_magazineClass, _turretPath];
						private _HzRoundsAdded = _currentRounds + _numRounds - _rounds;
						_vehicle setMagazineTurretAmmo [_magazineClass, _HzRoundsAdded, _turretPath];
						_HzCost = _HzCost + _HzRoundsAdded*_HzAmmoUnitCost;
					};
				} else {
					private _HzRoundsAdded = _currentRounds + _numRounds;
					_vehicle setMagazineTurretAmmo [_magazineClass, _HzRoundsAdded, _turretPath];
					_HzCost = _HzCost + _HzRoundsAdded*_HzAmmoUnitCost;
				};
				[QEGVAR(common,displayTextStructured), [[LSTRING(Hint_RearmedTriple), _numRounds,
				...
				if (_numMagazines  < _maxMagazines) then {
					...
					_HzCost = _HzCost + _currentRounds*_HzAmmoUnitCost;
				};
				[QEGVAR(common,displayTextStructured), [[LSTRING(Hint_RearmedTriple), _rounds,
				getText(configFile >> "CfgMagazines" >> _magazineClass >> "displayName"),
				getText(configFile >> "CfgVehicles" >> (typeOf _vehicle) >> "displayName")], 3, _unit], [_unit]] call CBA_fnc_targetEvent;
